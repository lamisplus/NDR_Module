...
export default function GenerateNdr(props) {
    ...
    const handleProgressComplete = () => {
        setShowProgressBar(false); // Hide progress bar when complete
        setModal(false); // Close modal when progress completes
        setProcessing(false); // Re-enable generate button
        toast.success('NDR generation completed!');
    };
    ...
    return (
        <div>
            ...
            >
                <ModalBody>
                    <h2>NDR File Generation in progress...</h2>
                    {showProgressBar && <ProgressBar onComplete={handleProgressComplete} />}
                </ModalBody>
            </Modal>
